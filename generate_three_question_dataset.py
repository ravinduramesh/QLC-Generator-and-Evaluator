...
DEFAULT_OUTPUT = "Lab9Responses_three_questions.csv"
DEFAULT_SHEET = None
DEFAULT_LIMIT = None
...
def process_workbook(input_path: Path, output_path: Path, sheet_name: str | None, limit: int | None) -> None:
    ...
    with output_path.open("w", encoding="utf-8-sig", newline="") as csv_file:
        ...
        for index, (anon_id, student_code) in enumerate(zip(anon_ids, submissions), start=1):
            ...
            for target_level, level_description in TARGET_LEVELS:
                generated_rows[target_level.lower()] = call_openai_for_question_generation(
                    client,
                    model,
                    student_code,
                    target_level,
            ...
            row = {
                ...
                "understand_question": normalize_text(generated_rows["understand"]["question"]),
                "understand_answer": normalize_text(generated_rows["understand"]["answer"]),
                "apply_question": normalize_text(generated_rows["apply"]["question"]),
                "apply_answer": normalize_text(generated_rows["apply"]["answer"]),
                "create_question": normalize_text(generated_rows["create"]["question"]),
                "create_answer": normalize_text(generated_rows["create"]["answer"]),
            }
            writer.writerow(row)
